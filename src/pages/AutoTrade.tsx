...
import {
  ...
  Home, RefreshCw, Shield, Zap, Eye, Anchor, Download, Upload, TrendingUp, TrendingDown, Gauge,
} from 'lucide-react';
...
export default function ProScannerBot() {
  ...
  const signalIntervalRef = useRef<NodeJS.Timeout>();
  ...
  /* ── Signal Forge: Compute digit stats for a symbol ── */
  const computeDigitStats = useCallback((ticks: number[], thresholdDigit: number) => {
    ...
    for (const [symbol, ticks] of tickMapRef.current.entries()) {
      ...
      if (mostAppearing <= 6) {
        ...
        allCandidates.push({
          ...
          extra: `Threshold ${threshold} | Most:${mostAppearing} 2nd:${secondMost}`
        });
      }
      if (mostAppearing >= 5) {
      ...
      if (riseRate > fallRate && riseRate > 0.52) {
        const strength = 0.6 + riseRate * 0.3;
        allCandidates.push({
          type: "Rise/Fall",
          name: "⬆️ RISE",
        ...
        const strength = 0.6 + fallRate * 0.3;
        allCandidates.push({
          type: "Rise/Fall",
          name: "⬇️ FALL",
          strength: Math.min(0.92, strength),
        ...
        const strength = 0.58 + overRate * 0.25;
        allCandidates.push({
          type: "Rise/Fall",
          name: "📈 RISE (over bias)",
          strength: Math.min(0.92, strength),
          symbol,
          detail: `Over zone dominance ${(overRate*100).toFixed(0)}%`,
          extra: `Over:${(overRate*100).toFixed(0)}% Under:${(underRate*100).toFixed(0)}%`
        });
      } else if (underRate > overRate && underRate > 0.55) {
        const strength = 0.58 + underRate * 0.25;
        allCandidates.push({
          type: "Rise/Fall",
          name: "📉 FALL (under bias)",
          strength: Math.min(0.92, strength),
          symbol,
          detail: `Under zone dominance ${(underRate*100).toFixed(0)}%`,
          extra: `Over:${(overRate*100).toFixed(0)}% Under:${(underRate*100).toFixed(0)}%`
        });
      }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(generateSignals, 2000);
    signalIntervalRef.current = interval;
  ...
  return (
    <div className="space-y-2 max-w-7xl mx-auto">
      {/* ── Signal Forge Cards (Top 4 Signals) ── */}
      {topSignals.length > 0 && (
        <div className="bg-gradient-to-r from-primary/5 via-transparent to-primary/5 rounded-xl p-3 border border-primary/20">
          <div className="flex items-center gap-2 mb-3">
            <Gauge className="w-4 h-4 text-primary" />
            <h2 className="text-sm font-bold text-foreground">🔥 SIGNAL FORGE · TOP 4 OPPORTUNITIES</h2>
            <Badge variant="outline" className="text-[9px]">Live Analysis</Badge>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            {topSignals.map((signal, idx) => (
              <div key={`${signal.symbol}-${signal.type}-${idx}`} 
                className="bg-card border-l-4 border-primary rounded-lg p-2.5 hover:shadow-lg transition-all">
                <div className="flex items-center justify-between mb-1">
                  <Badge variant="secondary" className="text-[8px] font-mono">{signal.symbol}</Badge>
                  <span className="text-[9px] font-bold text-primary">{Math.round(signal.strength * 100)}%</span>
                </div>
                <div className="text-sm font-bold flex items-center gap-1">
                  {signal.name}
                </div>
                <div className="text-[9px] text-muted-foreground mt-1 truncate">{signal.detail}</div>
                <div className="mt-1.5 h-1 bg-muted rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-primary to-profit rounded-full" 
      ...
      {/* ── Main 2-Column Layout (Rest of the bot UI remains exactly the same) ── */}
